...
Status: specifications owner-approved; red-first implementation in progress
...
8. Archive in dependency order: M2 → runtime contracts → `add-session-settings`. Each archive must sync its deltas into canonical `openspec/specs/`; then run `npm run docs:build`, strict validation, and the change-specific stale-term/source scans before starting the next archive.
9. Update `SPEC.md` and README only with actual archived outcomes. After the M2 archive, require the broad case-insensitive whole-word product-vocabulary ban from M2 task 10.0 over production `app/Sources/**/*.swift` and `scripts/generate.sh`. Separately scan canonical specs, `openspec/config.yaml`, AGENTS, SPEC, README, and only their generated counterparts (`docs/site/AGENTS.html`, `README.html`, `SPEC.html`, and `openspec-specs-*.html`) for the deprecated phrases `Edition capability registry`, `Edition gating seam`, `Edition-neutral storage`, `Lite grants`, `Lite edition`, `Pro edition`, `Enterprise edition`, and `Lite, Pro, and Enterprise`; exclude historical plans, active delta pages quoting old `FROM` names, and archived change records. Then run the final full gate and independent validator over the archived tree.
