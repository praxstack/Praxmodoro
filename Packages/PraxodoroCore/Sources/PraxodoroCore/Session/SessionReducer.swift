...
public enum SessionReducer {
  ...
  public static func reduce(
    ...
    if let classified = classifyBoundaryCallback(snapshot: snapshot, intent: command.intent) {
      return classified
    }
    ...
    case .completed:
      return reduceCompleted(snapshot: snapshot, command: command, context: context)
    }
  }
  ...
  private static func classifyBoundaryCallback(
    snapshot: SessionSnapshot,
    intent: SessionIntent
  ) -> ReductionOutcome? {
    let token: BoundaryToken
    switch intent {
    case let .openCheckIn(.scheduled(value)):
      guard value.kind == .scheduledCheckIn, value.phaseID == nil else {
        return .rejected(snapshot: snapshot, reason: .staleBoundary(value))
      }
      token = value
    case let .openCheckIn(.phaseBoundary(value)):
      guard value.kind == .phase, value.phaseID != nil else {
        return .rejected(snapshot: snapshot, reason: .staleBoundary(value))
      }
      token = value
    case let .reconcileTime(.deadlineFired(value)):
      token = value
    default:
      return nil
    }
    if token == snapshot.lastConsumedBoundaryToken {
      return .rejected(snapshot: snapshot, reason: .duplicateBoundary(token))
    }
    let installed: BoundaryToken? =
      switch token.kind {
      case .phase:
        if case let .focusing(focus) = snapshot.state { focus.phaseBoundaryToken } else { nil }
      case .scheduledCheckIn:
        snapshot.nextScheduledCheckIn?.token
      case .breakEnd:
        if case let .breaking(breakState) = snapshot.state { breakState.boundaryToken } else { nil }
      }
    guard installed == token else {
      return .rejected(snapshot: snapshot, reason: .staleBoundary(token))
    }
    return nil
  }
  ...
  private static func reduceCompleted(
    snapshot: SessionSnapshot,
    command: SessionCommand,
    context: ReductionContext
  ) -> ReductionOutcome {
  ...
  private static func reduceFocusing(
    ...
    case let .reconcileTime(observation):
      return reconcileLiveObservation(
        snapshot: snapshot,
  ...
  private static func reduceBreaking(
    ...
    if case let .parkThought(text) = command.intent {
      return parkThoughtInLiveState(snapshot: snapshot, text: text, context: context)
    }
    if case let .reconcileTime(observation) = command.intent {
      return reconcileLiveObservation(
        snapshot: snapshot,
        observation: observation,
        context: context
  ...
  private static func breakBoundaryTransition(
    ...
    parkedThought: ParkedThought? = nil
  ) -> ReductionOutcome {
    guard case let .breaking(breakState) = snapshot.state,
    ...
    let eventCount = UInt64(
      2 + (timing.admissionAdjustment == nil ? 0 : 1) + (parkedThought == nil ? 0 : 1))
    let remainingCapacity = UInt64.max - snapshot.eventSequence
    guard eventCount <= remainingCapacity else {
      return .failed(
        snapshot: snapshot,
    ...
    let candidate = SessionSnapshot(
      ...
      plan: snapshot.plan,
      configuration: snapshot.configuration,
      parkedThoughts: parkedThought.map { snapshot.parkedThoughts + [$0] }
        ?? snapshot.parkedThoughts,
      startedAt: snapshot.startedAt,
      accumulatedFocusSeconds: snapshot.accumulatedFocusSeconds,
      accumulatedBreakSeconds: accumulatedBreakSeconds,
      lastWallObservationAt: timing.observedWallNow,
    ...
    if let parkedThought { payloads.append(.thoughtParked(id: parkedThought.id)) }
    payloads.append(.breakEnded)
    payloads.append(.reentryPresented)
    let events = payloads.enumerated().map { offset, payload in
      SessionEvent(
  ...
  private static func focusBoundaryTransition(
    ...
    let eventCount = UInt64(
      payloads.count + (timing.admissionAdjustment == nil ? 0 : 1)
        + (parkedThought == nil ? 0 : 1))
    let remainingCapacity = UInt64.max - snapshot.eventSequence
    guard eventCount <= remainingCapacity else {
      return .failed(
        snapshot: snapshot,
    ...
    if let parkedThought { orderedPayloads.append(.thoughtParked(id: parkedThought.id)) }
  ...
  private static func parkThoughtInLiveState(
    snapshot: SessionSnapshot,
    text: String,
    context: ReductionContext
  ) -> ReductionOutcome {
    ...
    case let .failure(reason): return .failed(snapshot: snapshot, reason: reason)
    case let .recovery(reason):
      if case .focusing = snapshot.state {
        return focusRecoveryTransition(snapshot: snapshot, reason: reason, context: context)
      }
      return breakRecoveryTransition(snapshot: snapshot, reason: reason, context: context)
    }
    let thought = ParkedThought(
      id: context.generatedThoughtID,
      text: normalized,
      createdAt: timing.observedWallNow
    )
    switch SessionTimeKernel.admitBoundary(
      snapshot: snapshot,
      timing: timing,
    ...
    case let .winner(winner):
      if case .focusing = snapshot.state {
        return focusBoundaryTransition(
          snapshot: snapshot,
          timing: timing,
          winner: winner,
          parkedThought: thought
        )
      }
      return breakBoundaryTransition(
        snapshot: snapshot,
        timing: timing,
        winner: winner,
        parkedThought: thought
    ...
    let candidate = SessionSnapshot(
      ...
      plan: snapshot.plan,
      configuration: snapshot.configuration,
      parkedThoughts: snapshot.parkedThoughts + [thought],
      startedAt: snapshot.startedAt,
      accumulatedFocusSeconds: snapshot.accumulatedFocusSeconds,
      accumulatedBreakSeconds: snapshot.accumulatedBreakSeconds,
      lastWallObservationAt: timing.observedWallNow,
    ...
    var payloads: [SessionEventPayload] = []
    if let adjustment = materialization.adjustment { payloads.append(.clockAdjusted(adjustment)) }
    payloads.append(.thoughtParked(id: thought.id))
    let events = payloads.enumerated().map { offset, payload in
      SessionEvent(
        sessionID: sessionID,
        sequence: snapshot.eventSequence + UInt64(offset) + 1,
    ...
    let projectionToken: UUID? =
      switch state {
      case let .focusing(focus): focus.projectionToken
      case let .breaking(breakState): breakState.projectionToken
      default: nil
      }
  ...
  private static func reduceReentering(
    ...
    if case let .parkThought(text) = command.intent {
      return parkThoughtInNonLiveState(snapshot: snapshot, text: text, context: context)
    }
    if case let .requestBreak(choice) = command.intent {
      return startBreakFromReentry(snapshot: snapshot, choice: choice, context: context)
    }
    guard
      let configuration = requestedConfiguration(
        for: command.intent,
  ...
  private static func startBreakFromReentry(
    snapshot: SessionSnapshot,
    choice: BreakChoice,
    context: ReductionContext
  ) -> ReductionOutcome {
    ...
    let candidate = SessionSnapshot(
      ...
      state: .breaking(
        BreakState(
          ...
          projectionToken: materialization.projectionToken,
          boundaryToken: materialization.boundaryToken,
          resumeTarget: reentry.resumeTarget,
  ...
  private static func reconcileLiveObservation(
    snapshot: SessionSnapshot,
    observation: TimeObservation,
    context: ReductionContext
  ) -> ReductionOutcome {
    if observation == .relaunch {
    ...
    case let .recovery(reason):
      ...
      case .breaking:
        return breakRecoveryTransition(snapshot: snapshot, reason: reason, context: context)
      default:
        return invalidTransition(snapshot: snapshot, intent: .reconcileTime(observation))
      }
    }
    let observedToken: BoundaryToken? =
      if case let .deadlineFired(token) = observation { token } else { nil }
    switch SessionTimeKernel.admitBoundary(
      snapshot: snapshot,
      timing: timing,
      observedToken: observedToken
    ...
    case let .boundaryNotDue(token, dueAt, observedAt):
      if let materialization = timing.liveCommitMaterialization,
        let adjustment = materialization.adjustment
      {
        return commitLiveClockAdjustment(
          snapshot: snapshot,
          timing: timing,
  ...
  private static func commitLiveClockAdjustment(
    snapshot: SessionSnapshot,
    timing: NormalizedLiveTiming,
    materialization: LiveCommitMaterialization,
    ...
    let event = SessionEvent(
      ...
      sequence: snapshot.eventSequence + 1,
      occurredAt: timing.observedWallNow,
      payload: .clockAdjusted(adjustment)
    )
    var effects = notificationReplacementEffects(from: snapshot, to: candidate)
    let projectionToken: UUID? =
      switch state {
