...
Every surface SHALL render from a single engine-derived snapshot produced by the app model at one instant, and no surface SHALL hold, count, or decrement its own copy of the remaining time. Remaining time SHALL remain a pure function of the recorded transitions and the current instant.
...
- **WHEN** the user acknowledges the return overlay
...
### Requirement: Companion surfaces are never paywalled
Every companion surface SHALL consult the capability registry, and the keys backing them SHALL resolve as available in Lite; initiation, check-in entry, hold/resume, and accessibility behavior reached through these surfaces SHALL remain inside the never-paywalled set.

#### Scenario: Lite grants every companion surface
- **WHEN** the registry resolves the companion-surface feature keys under Lite
- **THEN** every key SHALL resolve to available and no surface SHALL render an upsell

#### Scenario: Companion surface keys cannot be gated off Lite
- **WHEN** a registry configuration attempts to mark a companion-surface key as non-Lite
...
- **WHEN** the companion-surface text and background tokens are rasterized under standard and increased contrast
- **THEN** the measured contrast ratio SHALL meet the normal-text minimum under standard settings and SHALL be strictly higher under increased contrast
