...
#### Scenario: Clean command-line build
...
#### Scenario: Repository contract suite
...
#### Scenario: Scene integration test
...
The repository SHALL provide commands and CI steps for strict OpenSpec validation, formatting, static analysis, secret scanning, core unit tests, repository tests, app build, UI/accessibility tests where automatable, and a launch smoke test.

#### Scenario: Pull-request workflow
- **WHEN** the macOS CI workflow runs for a proposed change
- **THEN** every configured gate executes on a supported macOS runner and the workflow fails if any required gate fails

#### Scenario: Local verification
- **WHEN** a developer runs the documented verification script
- **THEN** it executes the same critical spec, test, build, secret, and smoke gates without requiring production credentials
...
- **WHEN** a new agent or developer opens the repository without prior conversation context
- **THEN** the documented source order identifies the active change, next unfinished atom, exact commands, known failures, and acceptance evidence without relying on chat memory
