...
- [ ] 3.3 No-network/no-passive-data structural tests; red: `StoreAuditTests.testNoNetworkSymbolsLinked`, `testNoPassiveObservationFields`.
- [ ] 3.4 Corrupt-store recovery path (fresh store + preserved recovery file + plain notice); red: `testCorruptStoreStillReachesInitiate`.

## 4. Companion field physics

- [ ] 4.1 Port `FieldPhysics` value type from `design-mocks/living-companion/companion-physics.js` (springs, asymmetric breath 3.6/1.2/5.4/1.4, state targets, pulse); red: `FieldPhysicsGoldenTests.testTransformsMatchJSContractAtFixedTimestamps` against exported JS values; files: `app/Sources/CompanionField/FieldPhysics.swift`.
- [ ] 4.2 Canvas/TimelineView renderer with layered radial gradients; smoke: visual side-by-side with mock at breathing/held/expanded.
- [ ] 4.3 Total reduced-motion standdown (static alternate view, physics never instantiated); red: `testReduceMotionNeverInstantiatesPhysics`.

## 5. Core loop surfaces

- [ ] 5.1 Initiate surface (one task, first action, capacity, policy, ⌘↩ begin, nothing else on path); red: `InitiateSurfaceTests.testStartPathContainsOnlyLoopControls`; files: `app/Sources/Surfaces/InitiateSurface.swift`.
- [ ] 5.2 Focus surface (task+time legible, thought parking without context loss, no scoring elements); red: `testThoughtParkingKeepsFocusActive`, `testNoScoringElementsPresent`.
- [ ] 5.3 Check-in surface (four answers, timer held, non-grading responses, waits for field blur mid-keystroke); red: `testFourAnswersNoFailureState`, `testCheckinDefersWhileTyping`.
- [ ] 5.4 Break surface (user-steerable suggestions with provenance disclosure, re-entry card, early end is ordinary); red: `testEarlyEndHasNoPenaltyPath`, `testSuggestionDisclosesProvenance`.
- [ ] 5.5 Review surface (timeline from event log, uncertainty-aware insight copy); red: `testSingleDayInsightStatesLimits`.
- [ ] 5.6 App lifecycle restore (relaunch lands on the live surface from persisted state); red: `LifecycleTests.testRelaunchIntoRunningBlockShowsFocus`.

## 6. Cross-cutting gates

- [ ] 6.1 `CapabilityRegistry` + startup validation of the never-paywalled set; red: `testLiteGrantIsTotal`, `testCorePaywallAttemptFailsValidation`.
- [ ] 6.2 Copy-tone lint test over the strings catalog vs banned-claims lexicon; red: `CopyToneTests.testNoMedicalOrJudgmentClaims`.
- [ ] 6.3 Accessibility scenario suite (keyboard loop, VoiceOver field summary, Reduce Transparency solid alternates, Increase Contrast ratios); red: `AccessibilityTests` per focus-loop-ui spec scenarios.
- [ ] 6.4 Network-silence harness over a full loop run; red: `testFullLoopMakesZeroOutboundConnections`.
- [ ] 6.5 Provenance About surface (version, SHA, edition); red: `testAboutShowsProvenance`.
- [ ] 6.6 Full gauntlet: fresh focused tests, full suite, build, lint/format, smoke QA of the five-surface loop, `npm run spec:validate`, independent validator pass against SPEC.md; update `prd.json` + `progress.txt` with evidence.
