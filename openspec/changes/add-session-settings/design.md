...
5. **Sound is a transition listener.** `SoundPlayer` (AVFoundation) observes the same state changes surfaces render; ticking is an `AVAudioPlayer` loop started/stopped by state, never a `Timer`. The chime for an expiry that happened while asleep does not retro-fire — a sound after the fact is noise, not information. *As built:* chimes ARE scheduled ahead, via `AVAudioPlayer.play(atTime:)` on the device timebase (the `AVAudioEngine` flavour stays deferred). Known edge, accepted: the device timebase pauses across system sleep, so a chime pending at sleep can fire late on wake; the no-retro rule is enforced at every sync point, which bounds the staleness to one sleep window.
...
10. **Derived transitions sync presentation through the render edge.** Sound and notifications re-sync on every intent AND on the routing loop's phase-change edge (`onChange` over the derived phase) — presentation only, the session is never written there, so the one-clock rule stands. (Added for validator finding 3: without the edge, an enabled tick outlived a derived hold.)
11. **The caller decides when the rhythm continues.** `reconciled` materializes full autostart/auto-return cycles at canonical instants — pure and honest for a live app that reconciles continuously. On restore after an absence, the app passes `autoReturn: nil` for the catch-up reconcile, or the absence would fill with focus blocks nobody lived through.
